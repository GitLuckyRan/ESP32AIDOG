...
struct gait_state_t {
    float step_height {KinConfig::default_step_height+0.1};
    float step_x {0.0};
    float step_z {0.0};
    float step_angle {0};
...
class WalkState : public MotionState {
  private:
    ...
    // 替换掉原来的 combinatorial_constexpr 调用
  ...
  public:
    ...
    void step(body_state_t &body_state, float dt = 0.02f) override {
        ...
        target_body_state.ym = 0.15f;        // 站高 16 厘米
        target_gait_state.step_x = 0.03f;    // 步长 2.5 厘米
        target_gait_state.step_z = 0.0f;
        target_gait_state.step_velocity = 1.0f; // 步频 1Hz
        target_gait_state.step_height = 0.02f;  // 抬脚 4 厘米（原来 2cm 太低，后腿抬不起来）
        target_gait_state.step_depth = 0.02f;
        ...
        gait_state.step_z = lerp(gait_state.step_z, target_gait_state.step_z, smoothing_factor);
        gait_state.step_velocity = target_gait_state.step_velocity;
        gait_state.step_angle = lerp(gait_state.step_angle, target_gait_state.step_angle, smoothing_factor);
        gait_state.step_depth = lerp(gait_state.step_depth, target_gait_state.step_depth, smoothing_factor);
  ...
  protected:
    ...
    void updateBodyPosition(body_state_t &body_state, float dt) {
        ...
        if (leg_states.stance_count >= 3 && leg_states.swing_count == 0 && leg_states.next_swing != -1) {
            ...
            float smooth_progress = smoothstep01(clamp(progress, 0.0f, 1.0f));
    ...
    static void bezierCurve(const float length, const float angle, const float *height, const float phase,
        ...
        const float t = clamp(phase, 1e-4f, 1.f - 1e-4f);
